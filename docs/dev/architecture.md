...
The core of the tool revolves around 4 abstractions:

1. the napari plugin repository: `NapariPlugin`
2. the requirement: `Requirement`, representing a rule that needs to be checked
...
The `Engine` is here an abstract concept, not really implemented, that just represents the fact that it takes as input a `NapariPlugin` instance, a `RequirementSuite` instance and produces a `PluginAnalysisResult`, which represents the results of the application for the checklist of the plugin.
The `RequirementSuite` is identified by a title, and contains two distinct lists of rules `Requirement`.
Depending the relationship that holds the instance, the result will be displayed in a section in the screen or in the CSV.
A `RequirementSuite` can have two types of requirements:

1. The relationship `requirements` holds `Requirement` instances that needs to return `True` or `False` only as results.
2. The relationship `additionals` holds `Requirement` instances that return additional information in the form of a string.
...
The result of the execution of a requirement suite over a repository is represented by a `PluginAnalysisResult` instance.
This instance owns a list of `Feature` instances which are the result of a specific `Requirement`.
These `Feature` instances are linked to a `MetaFeature`, and uses the attribute `found` to express if a `Requirement` has been satisfied or not.
Moreover, they also define relationships towards `RepositoryFile` to point exactly in which file the information have been found, and all the files that have been scanned.
...
Those files are either python PyPI configuration files, documentation (README.md for example), source code files, GitHub configuration files, citation files, etc.
...
* `*.toml`, the libraries `tomli` and `tomli_w` are used. From a `.toml` file, they produce as "AST" a dictionary.
...
As for the library used to parse toml files, a dictionary is returned as AST.
...
It's an instance of `Document` where somewhere in it's children at any level (`children+`), the content of the children (`children+>content`) is actually matching the general `APA_REGEXP` regexp.
Finally, the result of the matching will be extracted and stored in the `raw_apa_match` variable.
...
This patched version exposes internal API, patches some exceptions handling and exposes a new way to interact with the internal pip API in a programmatic way.
...
There is, however, a known issue on Windows, for some plugins, the "installability check" (relying on `pip`) sometimes returns invalid results when it comes to check the installability on platforms others than Windows.
...
If there is no issues, so, all the requirements of the requirement suite are satisfied, the process stops there.
...
First, the engine automatically fixes the problem by modifying the implied files.
