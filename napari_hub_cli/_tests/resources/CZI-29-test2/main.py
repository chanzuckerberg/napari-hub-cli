...
def create_checklist(repo):
    ...
    if (name_metadata_cfgfile(cfg_scraped_text)) or (
        name_metadata_npe2file(repo, npe2_napari_file)
    ):
    ...
    if (summary_metadata_cfgfile(cfg_scraped_text)) or (
        summary_metadata_naparicfg(napari_cfg_scraped_text)
    ):
    ...
    if (sourcecode_metadata_cfgfile(cfg_scraped_text)) or (
        sourcecode_metadata_naparicfg(napari_cfg_scraped_text)
    ):
    ...
    if (author_metadata_cfgfile(cfg_scraped_text)) or (
        author_metadata_naparicfg(napari_cfg_scraped_text)
    ):
    ...
    if (bug_metadata_cfgfile(cfg_scraped_text)) or (
        bugtracker_metadata_naparicfg(napari_cfg_scraped_text)
    ):
    ...
    if (usersupport_metadata_cfgfile(cfg_scraped_text)) or (
        usersupport_metadata_naparicfg(napari_cfg_scraped_text)
    ):
    ...
    if (
        (video_metadata_cfgfile(longdescription_scraped_text))
        or (
            video_metadata_descriptionfile(description_scraped_text)
            or screenshot_metadata_descriptionfile(description_scraped_text)
        )
        or (screenshot_metadata_cfgfile(longdescription_scraped_text))
    ):
    ...
    if (usage_metadata_cfgfile(longdescription_scraped_text)) or (
        usage_metadata_descriptionfile(description_scraped_text)
    ):
    ...
    if (intro_metadata_descriptionfile(description_scraped_text)) or (
        intro_metadata_cfgfile(longdescription_scraped_text)
    ):
    ...
    console.print(
        ...
        style=summary_sentence_column_style,
    )
    console.print(
        "- Intro Paragraph ", intro_paragraph_check, style=intro_paragraph_column_style
    )
    console.print(
        "- Intro Screenshot/Video ",
        intro_video_or_screenshot_check,
        style=intro_video_or_screenshot_column_style,
    )
    ...
    console.print(
        ...
        style=support_channel_column_style,
    )
    console.print(
        "- Issue Submission Link ",
        issue_submission_check,
        style=issue_submission_column_style,
    )
    console.print(
        "- Display Name ", display_name_check, style=display_name_column_style
    )
    console.print("\n[ OPTIONAL ] ")
