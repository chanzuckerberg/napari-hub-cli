...
def get_body_node(self):
    ...
    if self.type == "suite":
        ...
        for x in self.children:
            if not isinstance(x, cst.PythonNode) and x.type not in (
                "newline",
                "keyword",
            ):
                body.append(x)
...
setattr(
    ...
    "decorator_names",
    property(
        lambda self: [d.children[1].get_code().strip() for d in self.get_decorators()]
    ),
)
setattr(
    cst.ClassOrFunc,
    "decorators",
    property(lambda self: [d.children[1] for d in self.get_decorators()]),
)
setattr(cst.Import, "names", property(lambda self: self.get_paths()[0]))
setattr(
    cst.Import,
    "alias",
    property(lambda self: ".".join(n.value for n in self.get_defined_names())),
)
setattr(
    cst.ImportFrom,
    "module",
    property(lambda self: ".".join(n.value for n in self.get_from_names())),
)
