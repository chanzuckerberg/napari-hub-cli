...
from iguala import match, regex as re
...
class PythonFile(object):
    ...
    def _check_import(self, import_name):
        m = match(self.__class__)[
            'ast>*': (match(ast.Import)[ 'names>name': re(import_name)]
                      | match(ast.ImportFrom)['module': re(import_name)]) @ "ast_node",
            'path': '@file'
        ]
        result = m.match(self)
        if result.is_match:
            nodes = [(import_name, b['file'], b['ast_node'].lineno) for b in result.bindings]
            return nodes
        return []
