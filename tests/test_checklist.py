...
from napari_hub_cli.checklist.projectquality import (
    INSTALLABLE_LINUX,
    NUMBER_DEPENDENCIES,
    suite_generator,
)
...
def test_project_quality_nodeps(test_repo):
    ...
    requirement = next(
        (r for r in suite.requirements if INSTALLABLE_LINUX in r.features), None
    )
    additional = next(
        (r for r in suite.additionals if NUMBER_DEPENDENCIES in r.features), None
    )
